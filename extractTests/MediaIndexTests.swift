...
struct MediaIndexTests {
  ...
  func addMediaFromMediaItemData() async throws {
  ...
  func preventDuplicateMediaItems() async throws {
    let schema = Schema([MediaItem.self])
    let config = ModelConfiguration(isStoredInMemoryOnly: true)
    let container = try ModelContainer(for: schema, configurations: config)
  ...
  func addEmptyMediaArray() async throws {
    let schema = Schema([MediaItem.self])
    let config = ModelConfiguration(isStoredInMemoryOnly: true)
    let container = try ModelContainer(for: schema, configurations: config)
  ...
  func mediaTypeMapping() async throws {
    let schema = Schema([MediaItem.self])
    let config = ModelConfiguration(isStoredInMemoryOnly: true)
    let container = try ModelContainer(for: schema, configurations: config)
  ...
  func mixedNewAndExistingItems() async throws {
    let schema = Schema([MediaItem.self])
    let config = ModelConfiguration(isStoredInMemoryOnly: true)
    let container = try ModelContainer(for: schema, configurations: config)
